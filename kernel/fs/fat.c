...
static int fat_init(void) {
	...
	fat_init_struct = (fs_type_t *) malloc(sizeof(fs_type_t));
	memset(fat_init_struct, 0, sizeof(fs_type_t));
	...
	sector_buffer = (void *) malloc(FAT_SECTOR_BUFFER_SIZE);//TODO: why it was aligned?
	memset(sector_buffer, 0, FAT_SECTOR_BUFFER_SIZE);

	return 0;
}

//~ module_init(fat_init); //TODO
...
static fs_superblock_t* fat_make_superblock(fs_superblock_t* superblock, ptable_entry_t* pt) {
	...
	fat_fs_info_t *fs_info = (fat_fs_info_t *) malloc(sizeof(fat_fs_info_t));
	memset(fs_info, 0, sizeof(fat_fs_info_t));
	superblock->fs_info = fs_info;

	// Copy BPB (512 bytes)
	fs_info->bpb = (fat_fs_bpb_t *) malloc(512);
	...
	fs_info->root_directory = (void *) malloc(fs_info->root_dir_length);
...
void fat_read_cluster(fs_superblock_t* superblock, uint32_t cluster, void* buffer, uint32_t buffer_size) {
	...
	while(true) {
		...
		// Handle disk read errors
		if(derr != kDiskErrorNone) {
			printk("fat", "Error reading file: 0x%X\n", derr);
			break;
		}
...
static uint32_t fat_get_next_cluster(fs_superblock_t* superblock, uint32_t cluster_value) {
	...
	if(fs_info->fat_type == 32) {
		...
		// Handle disk read errors
		if(derr != kDiskErrorNone) {
			printk("fat", "Error reading FAT: 0x%X\n", derr);
			return 0xFFFFFFFE;
		}
...
void* fat_read_directory(fs_superblock_t* superblock, char* path) {
	...
	void *readMem = (void *) malloc(0x8000);
